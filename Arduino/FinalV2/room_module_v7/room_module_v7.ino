...
Task tsk7(500, TASK_FOREVER, &ComputeCurrent);

const int ADCread = A7;
int ACSSensitivity = 100;
double VoltageP2P = 0;
double Vrms = 0;
double Irms = 0;
...
void SchedulingMode(int xbee_data_int[]){
  ...
  room_device_status_int[0] = xbee_data_int[1];
  for(int x=0; x<xbee_data_int[2]; x++)
    room_device_status_int[1+x] = xbee_data_int[3+x];
      ...
      if (conn) {
        if (RXdata.equals("0")){
          if(ble_status){
            ...
            ble_status = 0;
            //Alarm.timerOnce(60, TurnOffRooms);
            TurnOffRooms();
          }
        }
...
void SendRoomConsumption(){
  String rm_ID = String(EEPROM.read(12), HEX);
  String frame_type = String(4, HEX);
  String data_length = String((String(current_consump)).length(), HEX);

  String complete_frame = frame_type+rm_ID+data_length+String(current_consump);
  
  Serial1.print(complete_frame);
  Alarm.delay(50);

...
void PerformXBeeOperation(int xbee_data_int[], int xbee_data_length){
  ...
  /*else if(xbee_data_int[0]==14)
    ...
    if(EEPROM.read(25)==0){
      ...
      set_room_device_status[0] = xbee_data_int[1];
      for(int x=0; x<xbee_data_int[2]; x++)
        set_room_device_status[1+x] = xbee_data_int[3+x];
    }
  }
  else if(xbee_data_int[0]==18){
    if(EEPROM.read(25)==0)
      tsk5.enable();
  }
}

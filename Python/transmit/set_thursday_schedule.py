...
def SetThursdaySchedule():
	FileMain("Setting thursday schedule")

	sql_args = ["room_schedules_start_time, room_schedules_end_time", "room_schedules", "room_schedules_day like '%{}%'".format("Thu")]
	...
	if room_schedules_result != 0:
		for row in room_schedules_result:
			...
			schedule.every().thursday.at(start_time).do(ChangeStatusToOn)

			schedule.every().thursday.at(end_time).do(ChangeStatusToOff)
...
while True:
	...
	if os.path.isfile('thursday_stop_file'):
		os.remove('thursday_stop_file')
		break

	time.sleep(1)

FileMain("Thursday scheduling stopped")